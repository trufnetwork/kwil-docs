...
# Execute a SELECT statement with a NUMERIC parameter (note the quotes)
kwil-cli query "SELECT * FROM balances WHERE amount > $min" --param 'min:numeric(78,0)=1000'
```
...
```bash
# Correct - with quotes
kwil-cli query "SELECT * FROM balances WHERE amount = $amt" --param 'amt:numeric(78,0)=1000'
