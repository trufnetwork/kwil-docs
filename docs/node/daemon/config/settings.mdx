...
- `p2p.blacklist.auto_blacklist_duration`: Duration to blacklist peers that exhaust connection retries (default: `"1h"`). Uses human-readable duration strings. Set to `"0s"` or `"0"` for permanent blacklisting. This setting only applies to automatically blacklisted peers; manually blacklisted peers can be configured with custom durations via CLI commands.

##### Configuration Examples

```toml title=config.toml
# Default temporary blacklisting (1 hour)
[p2p.blacklist]
enable = true
