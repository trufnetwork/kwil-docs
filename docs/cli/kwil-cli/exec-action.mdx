...
# Execute with a NUMERIC type parameter (note the quotes to prevent shell interpretation)
# Assume the action signature TRANSFER($amount numeric(78, 0))
kwil-cli exec-action transfer 'numeric(78,0):1000.5'

# Execute with a named NUMERIC parameter (note the quotes)
kwil-cli exec-action transfer --param 'amount:numeric(78,0)=1000.5'

# Execute with a NUMERIC array parameter (note the quotes)
kwil-cli exec-action batch_transfer 'numeric(10,2)[]:[100.5, 200.75, 300.00]'
```

:::tip Shell Quoting
When using parameters with special characters like parentheses (e.g., `numeric(10,5)`, `uuid`, arrays with `[]`),
